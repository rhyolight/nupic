...
class SpatialPooler(object):
  ...
  def _getPermanencesFromColumn(self, column):
    ...
    segment = self._region.columns[column].segment
    for synapse in segment.synapses:
      inputIndex = synapse.presynapticCellIndex
      perm[inputIndex] = synapse.permanence
  ...
  def _updatePermanencesForColumn(self, perm, index, raisePerm=True):
    ...
    for synapse in segment.synapses:
      inputIndex = synapse.presynapticCellIndex
      synapse.permanence = perm[inputIndex]
  ...
  def _getInputIndicesFromSynapsesForColumn(self, index, onlyConnected=False):
    ...
    inputIndices = [synapse.presynapticCellIndex for synapse in synapses]
  ...
  def _createSynapsesForColumn(self, potential, index):
    ...
    for inputIndex in selectedIndices:
      segment.createSynapse(presynapticCellIndex=inputIndex,
                            permanence=0.)
  ...
  def _calculateOverlap(self, inputVector):
    ...
    @param inputVector: a numpy array of 0's and 1's that comprises the input to
                    the spatial pooler.
    """
    overlaps = numpy.zeros(self._region.numColumns).astype(realDType)
    inputVectorNZ = numpy.nonzero(inputVector)
    for i in xrange(self._region.numColumns):
      inputIndices = self._getInputIndicesFromSynapsesForColumn(i,
                                                                onlyConnected=True)
      overlaps[i] = len(inputVector[inputVectorNZ])
